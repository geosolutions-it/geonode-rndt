...
(function () {
    ...
    // Load group categories
    module.load_pubblica_amministrazione = function ($http, $rootScope, $location) {
        var params = typeof FILTER_TYPE == 'undefined' ? {} : {'type': FILTER_TYPE};
        $http.get(PUBBLICA_AMMINISTRAZIONE_ENDPOINT, {params: params}).then(successCallback, errorCallback);

        function successCallback(data) {
            //success code
            $rootScope.pubblica_amministrazione = data.data.objects;
            if (HAYSTACK_FACET_COUNTS && $rootScope.query_data) {
                module.haystack_facets($http, $rootScope, $location);
            }
        };
    ...
    */
    module.run(function ($http, $rootScope, $location) {
        ...
        */
        ...
        module.load_pubblica_amministrazione($http, $rootScope, $location);
