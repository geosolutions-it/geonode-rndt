...
from geonode.layers.views import _PERMISSION_MSG_METADATA, _resolve_dataset, check_keyword_write_perms
    ...
    **kwargs,
    ...
    if request.method == "POST":
        ...
        if not constraint_form.is_valid():
            ...
            out = {
                "success": False,
                "errors": [re.sub(re.compile("<.*?>"), "", str(err)) for err in constraint_form.errors],
            }
        ...
        layer_constraint = items["free_text"] if items["use_constraints"] == "freetext" else items["use_constraints"]
        ...
        if layer_constraint.isnumeric():
            ...
            request.POST["resource-constraints_other"] = keyword.about
        else:
